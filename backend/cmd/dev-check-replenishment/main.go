...
func main() {
	...
	flag.StringVar(&asOfDate, "as-of-date", "", "optional date in YYYY-MM-DD; defaults to today UTC")
	flag.Parse()
	...
	service := analytics.NewReplenishmentService(postgres.Pool)
	result, err := service.ListReplenishmentForSellerAccount(ctx, sellerAccountID, asOf)
	if err != nil {
		fmt.Fprintf(os.Stderr, "list replenishment failed: %v\n", err)
		os.Exit(1)
	}
