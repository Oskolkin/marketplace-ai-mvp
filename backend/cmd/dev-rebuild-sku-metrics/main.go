...
func main() {
	...
	service := analytics.NewSKUMetricsService(postgres.Pool)
	if dateFrom != "" || dateTo != "" {
		if dateFrom == "" || dateTo == "" {
		...
		if _, err := service.RebuildDailySKUMetricsForDateRange(ctx, sellerAccountID, from, to); err != nil {
			fmt.Fprintf(os.Stderr, "rebuild sku metrics by range failed: %v\n", err)
			os.Exit(1)
		}
		fmt.Printf("daily_sku_metrics rebuilt for seller_account_id=%d in range %s..%s\n", sellerAccountID, dateFrom, dateTo)
		return
	}

	if err := service.RebuildDailySKUMetricsForSellerAccount(ctx, sellerAccountID); err != nil {
		fmt.Fprintf(os.Stderr, "rebuild sku metrics failed: %v\n", err)
		os.Exit(1)
	}
	fmt.Printf("daily_sku_metrics rebuilt for seller_account_id=%d across all source dates\n", sellerAccountID)
}
