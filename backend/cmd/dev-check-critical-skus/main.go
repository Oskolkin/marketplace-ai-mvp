...
func main() {
	...
	service := analytics.NewCriticalSKUService(postgres.Pool)
	result, err := service.ListCriticalSKUsForSellerAccount(ctx, sellerAccountID, asOf)
	if err != nil {
		fmt.Fprintf(os.Stderr, "list critical skus failed: %v\n", err)
		os.Exit(1)
	}
