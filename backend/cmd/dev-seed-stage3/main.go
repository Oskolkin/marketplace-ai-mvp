package main

import (
	"context"
	"flag"
	"fmt"
	"os"

	"github.com/Oskolkin/marketplace-ai-mvp/backend/internal/config"
	"github.com/Oskolkin/marketplace-ai-mvp/backend/internal/db"
	"github.com/Oskolkin/marketplace-ai-mvp/backend/internal/devseed"
)

func main() {
	var sellerAccountID int64
	flag.Int64Var(&sellerAccountID, "seller-account-id", 0, "target seller_account_id for synthetic seed (required)")
	flag.Parse()

	if sellerAccountID <= 0 {
		fmt.Fprintln(os.Stderr, "seller-account-id is required and must be > 0")
		os.Exit(2)
	}

	_ = config.LoadEnvFiles()
	cfg, err := config.Load()
	if err != nil {
		fmt.Fprintf(os.Stderr, "load config: %v\n", err)
		os.Exit(1)
	}

	ctx := context.Background()
	postgres, err := db.New(ctx, cfg.DB.URL)
	if err != nil {
		fmt.Fprintf(os.Stderr, "connect postgres: %v\n", err)
		os.Exit(1)
	}
	defer postgres.Close()

	result, err := devseed.SeedStage3ForSeller(ctx, postgres.Pool, sellerAccountID)
	if err != nil {
		fmt.Fprintf(os.Stderr, "seed failed: %v\n", err)
		os.Exit(1)
	}

	fmt.Printf("stage3 synthetic seed completed for seller_account_id=%d\n", result.SellerAccountID)
	fmt.Printf("products=%d orders=%d sales=%d stocks=%d\n", result.Products, result.Orders, result.Sales, result.Stocks)
}
